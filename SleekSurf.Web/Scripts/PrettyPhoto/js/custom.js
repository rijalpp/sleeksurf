...
// -------------------------------------------------------------------------------------------
	...
	jQuery('.ClientNavigation').k_nav_pointer({ fx: "easeOutQuint", speed: 700 });
	...
	jQuery(".blockslider").kriesi_block_slider({
			...
			appendControlls: '.featureWrapper',	//element to append the controlls to
	...
	flowplayer(".videoplayer", {src: "flashplayer/flowplayer-3.1.5.swf", wmode: "transparent", cachebusting: jQuery.browser.msie}, { // "videoplayer" is the class the player gets applied to
		clip: {  
	        autoPlay: true,
	        autoBuffering: true 
	    },
...
(function($)
{
	$.fn.kricordion = function(options) 
	{
		...
		{	
			...
			jQuery('.featuredInside').kriesi_image_preloader({callback:add_functionality});
...
function k_menu()
{
	...
	jQuery(".ClientNavigation li a").removeAttr('title');
	jQuery(" .ClientNavigation ul li").css({display: "none"}); // Opera Fix

	
	//smooth drop downs
	jQuery(".ClientNavigation li").each(function()
	{	
...
$.fn.k_nav_pointer = function(o) {
    ...
    return this.each(function() {
        ...
        $li.not(".back").not(".ClientNavigation ul li").hover(function() {
        	if(applytoall) 
        	{	
        ...
        $(".ClientNavigation li").hover(noop, function() {
            move(curr);
        });
